...
arraylist_t* arraylist_InitArraylist() {
	...
	if(arraylist == 0) return 0; //Out of memory
...
int arraylist_Add(cell_t cell, int index, arraylist_t* arraylist) {
	...
	if(arraylist->numCells == arraylist->size) { //The array is full
		if(arraylist_IncreaseSize(arraylist) < 0) { //IncreaseSize failed
			return -2;
		}
	}
	int i;
